...
function getCalendarId(deptid){
  return (deptid==2?config.get('gcalendar_id_all'):config.get('gcalendar_id_dev'));
}
...
gcal.prototype.promise_leave_request_decision_gevent = function(args){
  ...
  if(leave.status === Models.Leave.status_approved() || leave.status === Models.Leave.is_pended_revoke_leave())
  {
