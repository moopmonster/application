...
async function getCalendarId(deptid) {
  let gcal_id = "";
  let sql = "select a.gcal_id as 'dept_gcal', b.gcal_id as 'comp_gcal' from Departments a inner join Companies b on a.companyId = b.id where a.id = " + deptid;
  
  await Models.sequelize.query( sql , { 
    type: Models.sequelize.QueryTypes.SELECT
  }).then((res) => {
    if(res[0].comp_gcal != null && res[0].comp_gcal != '')
      gcal_id = res[0].comp_gcal;
    else if (res[0].dept_gcal != null && res[0].dept_gcal != '')
      gcal_id = res[0].dept_gcal;
...
gcal.prototype.promise_gevent_create = async function(args){
  var self   = this,
  leave      = args.leave;

  var leave_event = constructEvent(leave);
  var department_id = leave.user.DepartmentId;
  var calid = await getCalendarId(department_id);
...
gcal.prototype.promise_leave_request_decision_gevent = async function(args){
  var self          = this,
  leave             = args.leave;
  var calid = await getCalendarId(leave.user.DepartmentId);
