...
TeamView.prototype.promise_team_view_details = function(args){
  ...
  let
    ...
    current_company_id = args.company_id, // optional parameter
    related_departments = [],
    current_department,
  ...
  var normalise_departments_func = function(my_department, supervised_departments){
    ...
    if (current_company_id) {
      current_company_departments = _.filter(supervised_departments, function(department){ return department.companyId == Number(current_company_id) });
    }


    return Promise.resolve(current_department ? [current_department] : current_company_departments ? current_company_departments : supervised_departments);
    // if (current_department != NULL){
    //   [current_department];
    // }
    // else if (current_company_departments != NULL)
    // {
    //   [current_company_departments];
    // }
    // else{
  ...
  if (user.is_superAdmin()){
    promise_departments = user.Model.sequelize.models.Department.findAll()
      .then(function(departments){
        return normalise_departments_func(null, departments);
      });
  }
  else if (user.is_admin() || user.company.share_all_absences) {
  ...
  team_view_details
    ...
    .forEach(node => {
      ...
      node
        ...
        // Ignore weekends
         // .filter( day => ! day.is_weekend ) Remove this filter due to leave adjustment could be on weekend
        // Ignore bank holidays
         //.filter( day => ! day.is_bank_holiday ) Remove this filter due to leave adjustment could be on public holiday
        .forEach( day => {
