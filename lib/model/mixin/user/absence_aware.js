...
module.exports = function(sequelize){
  ...
  this.get_leave_statistics_by_types = function(args){
    ...
    var sick_leave_limit = this._get_sick_leave_limit();
    ...
    this.company.leave_types.forEach(function(leave_type){
      ...
      var lt_limit = (leave_type.name=="Sick Leave"?sick_leave_limit:leave_type.limit);
  ...
  this.validate_leave_fits_into_remaining_allowance = function(args){
    ...
    .then(function(employee){
      ...
      var lt_limit = (leave_type.name=="Sick Leave"?self._get_sick_leave_limit():leave_type.limit);
  ...
  this.promise_to_add_Annual_Leave = function(args) {
    ...
    var
      ...
      .findOrCreate({
            ...
            defaults : {
              userId           : employee.id,
              leaveTypeId      : 10, //Temp Hardcode as + Annual Leave Type
              status           : 2, //Temp Hardcode as Approved leave
              verified         : 1, //Temp hardcode as verified leave
              verifiedBy       : user.id,
              approverId       : user.id,
        ...
        .update(
            {user_leave: parseInt(employee.user_leave) + 1},
            {where:
              {id: employee.id}
            }
        )
