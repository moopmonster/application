...
const
  ...
  schema_year = Joi
    ...
    //@@2019 hardcode@@
    .default(() => moment.utc("2019-01-01"), 'Default year is current one'),
  ...
  schema_now = Joi
    ...
    //@@2019 hardcode@@
    .default(() => moment.utc("2019-01-01"), 'Default is UTC one not from company specific'),
  ...
  scheme_constructor = Joi.object().required().keys({
    ...
    //
   // TODO now needs to replace year parameter (year should just be derived from now)
    now : schema_now,
  });
...
class UserAllowance {
  ...
  get accrued_adjustment() {
    ...
    let allowance = self.nominal_allowance
      + self.manual_adjustment
      + self.employement_range_adjustment;

    let period_starts_at = moment.utc(self.user.start_date).year() === now.year()
      ? moment.utc(self.user.start_date)
      : now.clone().startOf('year');

    let period_ends_at = self.user.end_date && moment.utc(self.user.end_date).year() <= now.year()
      ? moment.utc(self.user.end_date)
      : now.clone().endOf('year');
    ...
    return Promise
      ...
      .then(() => {
        return new UserAllowance({
          ...
          nominal_allowance                   : user.user_leave,
        });
      });
  }
}
