...
const
  ...
  scheme_constructor = Joi.object().required().keys({
    ...
    allowanceBasedOnYear : Joi.number().required(),
    initial_adjustment : Joi.number().required(),
    now : schema_now,
  });
...
class UserAllowance {
  ...
  constructor(args) {
    ...
    self._now                                 = args.now;
    self._allowanceBasedOnYear                = args.allowanceBasedOnYear;
    self._initial_adjustment                   = args.initial_adjustment;
  }
  ...
  get total_number_of_days_in_allowance_with_year() {
    const self = this;

    return (self._allowanceBasedOnYear +
      self.carry_over +
      self.manual_adjustment +
      self.initial_adjustment +
      self.employement_range_adjustment);
  ...
  get initial_adjustment() {
    return this._initial_adjustment;
  }
  ...
  get allowanceBasedOnYear() {
    return this._allowanceBasedOnYear;
  }
  ...
  get number_of_days_available_in_allowance_with_year() {
    const self = this;
    ...
    return (
      self.total_number_of_days_in_allowance_with_year
      - self.number_of_days_taken_from_allowance
      + (self.is_accrued_allowance ? self.accrued_adjustment : 0)
    );
  }
  ...
  static promise_allowance(args) {
    ...
    flow = flow.then(adjustment_and_coa => {
      ...
      initial_adjustment     = adjustment_and_coa.initial_adjustment;
    ...
    flow = flow.then(() => Promise.resolve(
        allowanceBasedOnYear = Math.max(user.user_leave,user.calculated_userleaves)
    ));
    ...
    flow = flow.then(() => {
      ...
      const args = {
        ...
        nominal_allowance: user.get_highest_user_leaves(), //current year
        allowanceBasedOnYear : allowanceBasedOnYear,
        initial_adjustment : initial_adjustment,
      };

      if ( year && year.year() !== moment.utc().year() ) {
