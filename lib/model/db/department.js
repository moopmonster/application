    ...
    is_accrued_allowance : {
      ...
      instanceMethods : {
        ...
        _promise_team_view : function(args){
          ...
          var deptdata = JSON.parse(JSON.stringify(self));
          for (var key in deptdata) {
            if(key === "id")
              departmentsMap[ deptdata["id"] ] = "";
            if(key === "name")
              departmentsMap[ deptdata["id"] ] = deptdata["name"];  
          }
                  ...
                  .then(function(leaves){
                    ...
                    return user.promise_schedule_I_obey()
                      .then(function(schedule){
                        return Promise.resolve({
                          user       : user,
                          department : departmentsMap[ user.DepartmentId ],
