...
get_deducted_days : function(args) {
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      if (leave_type.is_adjustment==0 && bank_holiday_map[ leave_day.get_pretty_date() ] ) return;
      ...
      if (leave_type.is_adjustment==0 && ! schedule.is_it_working_day({ day : moment.utc(leave_day.date) }) ){
        return;
      }
...
get_leave_type_attachment_is_image : function() {
  var self = this;
  if((self.attachment != null)&&(path.extname(self.attachment).toUpperCase() == '.JPG' || path.extname(self.attachment).toUpperCase() == '.PNG')){
    return 1;
  }else{
    return 0;
  }
