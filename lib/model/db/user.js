...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    promise_companies_I_can_manage : function(){
      ...
      if ( this_user.is_superAdmin()){
        return sequelize.models.Company.findAll(
        {
            ...
            order : [
              [{model : sequelize.models.LeaveType, as : 'leave_types'}, 'id']
            ],
        }
        ) ;
      }else if ( this_user.is_admin() ) {
    ...
    promise_to_return_AL_entitlement : function(year){
      ...
      if (calculateALEntitlement(year_of_service) == calculateALEntitlement(partial_year_of_service)){
        return calculateALEntitlement(year_of_service);
      }else{ 
        let 
        ...
        leave_entitle_for_full_year = calculateALEntitlement(year_of_service)*((date_to_calcalate.diff(first_day_of_year,'days')+1)/days_in_year); //add one day to include end date
        partial_year_of_service = calculateALEntitlement(partial_year_of_service)*(Last_day_of_year.diff(date_to_calcalate,'days')/days_in_year); //no need to add one day because date_to_calcalate already include end-date for this part
      ...
      function calculateALEntitlement(year_of_service){
