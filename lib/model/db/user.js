  ...
  return {
    ...
    promise_to_return_AL_entitlement : function(year){
      ...
      if (calculateALEntitlement(year_of_service) == calculateALEntitlement(partial_year_of_service)){
        ...
        leave_entitle_for_full_year = calculateALEntitlement(year_of_service)*((date_to_calcalate.diff(first_day_of_year,'days')+1)/365); //add one day to include end date
        partial_year_of_service = calculateALEntitlement(partial_year_of_service)*(Last_day_of_year.diff(date_to_calcalate,'days')/365); //no need to add one day because date_to_calcalate already include end-date for this part
