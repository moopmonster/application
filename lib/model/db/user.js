  ...
  return {
    ...
    promise_to_return_AL_entitlement : function(year){
      let self = this,
          ...
          year_of_service = Math.floor((date_to_calcalate.diff(joinedDate,'days')+1)/365),
          partial_year_of_service = Math.floor(year_of_service + 1);
      ...
      .then(function(Country){
        if (calculateALEntitlement(year_of_service,Country) == calculateALEntitlement(partial_year_of_service,Country))
        {
          ...
          if (remainder > 0 && remainder < 0.1){
            totalALentilement = Math.floor(totalALentilement)
          }
          else if (remainder >= 0.1 && remainder <= 0.5){
      ...
      function calculateALEntitlement(year_of_service,country){
        ...
        if (country == 'UAE'){
           if (year_of_service <= 1){
            total_AL_entitlment = 24;
          }else{
            total_AL_entitlment = 30;
          }
        }
        else if (country == 'TH' || country == 'VN' || country == 'PH'){
        ...
        else if (country == 'HK' || country == 'ID' || country == 'SG'){
          if (year_of_service <= 2){
            total_AL_entitlment = 15;
          }else
          if (year_of_service > 2 && year_of_service <= 4){
