...
router.get('/allowanceclosing/', (req,res) => {
  ...
  let users_filter = {};
  let companies_filter = {id: req.user.companyId}; //default as request user's company id
  ...
  let currentYear = moment.utc(end_date, 'YYYY') //Get report year
  ...
  if (validator.isNumeric( req.param('company') )) {
    companies_filter = { id: req.param('company') };
    current_company_id = req.param('company');
  } else {
    current_company_id = req.user.companyId;
  }
    ...
    .then(company => Promise
      ...
      .map(user => user
        .promise_allowance({year: currentYear})
...
function promise_user_list_finance_report(args) {
  let
    company = args[0],
    users_info = args[1],
    report_start_date = args[2],
    report_end_date = args[3];
  ...
  let users_info_for_rendering = users_info.map(ui => ({
    ...
    user_name                             : ui.user_row.name,
    user_full_name                        : ui.user_row.full_name(),
    user_staff_id                         : ui.user_row.staffId,
    user_category                         : ui.user_row.category,

    total_number_of_days_in_allowance_with_prorated_initial_adjustment : ui.total_number_of_days_in_allowance_with_prorated_initial_adjustment,
    manual_adjustment                           : ui.manual_adjustment,
    carry_over                                  : ui.carry_over,
    nominal_allowance                           : ui.nominal_allowance,
    initial_adjustment                          : ui.initial_adjustment,
    annual_Leave_taken                          : ui.user_row.calculate_number_of_days_taken_from_allowance(),
    sick_Leave_taken                            : _.filter(ui.leave_statistics,function(leaveType){return leaveType.leave_type.leaveTypeCode == 'SICK'})[0].days_taken,

    user_dtjoin                           : moment(ui.user_row.start_date).format('YYYY-MM-DD'),
    user_dtleave                          : moment(ui.user_row.end_date).format('YYYY-MM-DD'),
    is_resigned                           : (ui.user_row.end_date != null && moment(ui.user_row.end_date).format('YYYY-MM-DD') <  moment(report_start_date).startOf('day').format('YYYY-MM-DD')? 1:0),
...
router.get('/allowancebytime/', (req, res) => {
  ...
  var current_company_id  = validator.isNumeric(req.query['company'])
    ? req.query['company']
    : null;

  var current_deparment_id  = validator.isNumeric(req.query['department']) && req.query['department'] > 0
  ...
  Promise.join(
    ...
    req.user.get_company_with_all_leave_types(current_company_id),
    (departments,team_view_details, company) => {
      return team_view
        .inject_statistics({
        ...
        .then(team_view_details => render_allowancebytime({
          ...
          res               : res,
          team_view_details : team_view_details,
          departments       : departments, //for superadmin usage
          company           : company,
          start_date        : start_date,
          end_date          : end_date,
        }))
...
router.get('/allowancefinance/', (req, res) => {
    let model = req.app.get('db_model');
  let users_filter = {};
  let companies_filter = {id: req.user.companyId}; //default as request user's company id
  ...
  if (validator.isNumeric( req.param('company') )) {
    ...
    .then(company => Promise
      ...
      .map(user => user
        .promise_allowance({year: currentYear})
        .then(allowance_obj => Promise.resolve({
          leave_statistics                                                 : user.get_leave_statistics_by_types(),
          user_row                                                         : user,
          total_number_of_days_in_allowance_with_prorated_initial_adjustment : allowance_obj.total_number_of_days_in_allowance_with_year - allowance_obj.carry_over - allowance_obj.manual_adjustment,
          manual_adjustment                                                : allowance_obj.manual_adjustment,
          carry_over                                                       : allowance_obj.carry_over,
          nominal_allowance                                                : allowance_obj.nominal_allowance,
          initial_adjustment                                               : allowance_obj.initial_adjustment,
          // replacementLeave                      : allowance_obj.replacementLeave,
        })),
        {
          concurrency : 10
        }
      )
      .then(users_info => Promise.resolve([company, users_info,start_date,end_date]))
    )
    .then(args => promise_user_list_finance_report(args))
    .then(function(args){
      let
        company = args[0],
        users_info = args[1];
      ...
      if ( req.param('as-csv') ) {
        return render_allowancefinance_as_csv({
          req            : res,
          res            : res,
          company        : company,
          department_id  : Number(current_deparment_id),
      ...
      res.render('report/allowancefinance', {
        company       : company,
        department_id : Number(current_deparment_id),
        users_info    : users_info,
        start_date_str : start_date.format("YYYY-MM-DD"),
  ...
  let content = [
    ...
    .concat(
      team_view_details.users_and_leaves.length > 0
        ? team_view_details.users_and_leaves[0].statistics.leave_type_break_down.pretty_version.map(it => it.name.replace('+',''))
        : []
    )
...
function render_allowancefinance_as_csv(args) {
  ...
  // Compose result CSV header
  let content = [
    ['Staff ID', 'Full Name', 'Category', 'Joined date', 'Resigned date',  'Allowance(Entitlement)', 'Manual Adjustment', 'Carry Over(LeaveB/F)', 'Annual Leave taken', 'Sick Leave Taken']
  ];
  ...
  users_info.forEach(ui => {
    content.push([
      ...
      ui.total_number_of_days_in_allowance_with_prorated_initial_adjustment,
      ui.manual_adjustment,
      ui.carry_over,
      ui.annual_Leave_taken,
      ui.sick_Leave_taken
    ]);
  });
