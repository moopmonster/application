...
function promise_user_list_finance_report(args) {
  ...
  let users_info_for_rendering = users_info.map(ui => ({
    ...
    total_number_of_days_in_allowance_with_year : ui.total_number_of_days_in_allowance_with_year,
    manual_adjustment                           : ui.manual_adjustment,
    carry_over                                  : ui.carry_over,
    nominal_allowance                           : ui.nominal_allowance,
    initial_adjustment                          : ui.initial_adjustment,
    annual_Leave_taken                          : _.filter(ui.leave_statistics,function(leaveType){return leaveType.leave_type.leaveTypeCode == 'ANNUAL'})[0].days_taken,
    ...
    .then(company => Promise
      ...
      .map(user => user
        ...
        .then(allowance_obj => Promise.resolve({
          ...
          user_row                                    : user,
          total_number_of_days_in_allowance_with_year : allowance_obj.total_number_of_days_in_allowance_with_year,
          manual_adjustment                           : allowance_obj.manual_adjustment,
          carry_over                                  : allowance_obj.carry_over,
          nominal_allowance                           : allowance_obj.nominal_allowance,
...
function render_allowancefinance_as_csv(args) {
  ...
  users_info.forEach(ui => {
    content.push([
      ...
      ui.total_number_of_days_in_allowance_with_year,
      ui.manual_adjustment,
      ui.carry_over,
