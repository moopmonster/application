...
router.post('/credit_additional_leave/', (req, res) => {

  const
    days_credit_additional_leave = validator.trim(req.param('days_credit_additional_leave'));
    if ( ! validator.isNumeric(days_credit_additional_leave)) {
      req.session.flash_error('Number of leaves has to be a number');
    }
  ...
  req.user
    .getCompany()
    .then(company => creditAdditionalLeaves({company},days_credit_additional_leave))
    .then(() => req.session.flash_message(
