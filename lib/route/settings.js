...
router.post('/company/:company_id?/', function(req, res){
  ...
  const
    company_id        = typeof req.body('company_id') === 'undefined' || req.body('company_id') === null?req.user.companyId:validator.trim(req.body('company_id')), //if not defined then take the company id from the logged in user
    ...
    gcal_id           = validator.trim(req.body('gcal_id')),
...
function get_and_validate_leave_type(args) {
  ...
  let
    ...
    comment_req     = validator.toBoolean(req.body('comment_req__'+suffix))   || 0,
    min_days_prior  = validator.trim(req.body('min_days_prior__'+suffix))|| 0,
    attachment_opt  = validator.trim(req.body('attachment_opt__'+suffix)) || 0,
    attachment_req  = (attachment_opt==2 && validator.toBoolean(req.body('attachment_req__'+suffix))? 2: validator.toBoolean(req.body('attachment_req__'+suffix)))|| 0,
    use_allowance   = validator.toBoolean(req.body('use_allowance__'+suffix)),
    sort_order      = validator.trim(req.body('sort_order__'+suffix))  || 999,
    is_include_weekend_and_hols    = validator.trim(req.body('include_weekend_and_hols__'+suffix)) || 0;
  ...
  model.Company.findOne({
    ...
    .then(function(company){
      _.map(company.users,function(user){
        // console.log(users);
        user.update({
          calculated_userleaves : user.promise_to_return_AL_entitlement(curryear),
        })
        .then(function(user){
