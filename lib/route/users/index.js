...
router.post('/import-sample/:company_id/', function(req, res){
  ...
  // req.user
    ...
    .then(company => {
      ...
      content.unshift( ['email','lastname', 'name', 'nickname', 'department','start date ('+company.get_default_date_format()+')','AL entitlment','Carry Forward'] );
...
router.get('/', function(req, res) {
    ...
    if (req.user.superadmin != true){ //Only return the employee list that within the same company for the normal admin
    ...
    .then(function(result){
      ...
      // if ( req.param('as-csv') ) { //Temporary disable
...
function get_and_validate_user_parameters(args) {
    ...
    var name     = validator.trim(req.params['name']),
        lastname = validator.trim(req.params['lastname']),
        nickname = validator.trim(req.params['nickname']),
        email    = validator.trim(req.params['email_address']),
        department_id     = validator.trim(req.params['department']),
        start_date        = validator.trim(req.params['start_date']),
        end_date          = validator.trim(req.params['end_date']),
        adjustment        = validator.trim(req.params['adjustment']),
        password          = validator.trim(req.params['password_one']),
        password_confirm  = validator.trim(req.params['password_confirm']),
        admin             = validator.toBoolean(req.params['admin']),
        isManager         = validator.toBoolean(req.params['supervisor']),
        auto_approve      = validator.toBoolean(req.params['auto_approve']),
        carried_over_allowance = validator.trim(req.params['carried_over_allowance']),
        user_leave      = validator.trim(req.params['user_leave']);
        calculated_sickleaves      = validator.trim(req.params['user_sick_leave']);
    ...
    var attributes = {
        ...
        calculated_sickleaves : calculated_sickleaves,
    };
