...
function getCompanyId(req)  {
  return typeof req.params['company_id'] === 'undefined' || req.params['company_id'] === null?req.user.companyId:validator.trim(req.params['company_id']);
}
...
router.get('/bankholidays/:company_id?/', (req, res) => {
  ...
  if (!req.user.is_supervised_company(company_id)){ // prevent user to hijack the URL parameter
    ...
    return res.redirect_with_session('/settings/bankholidays/'+req.user.companyId);
  }
...
router.post('/bankholidays/:company_id?/', function(req,res){
  ...
  .then(() => {
    return res.redirect_with_session(`/settings/bankholidays/${company_id}/?year=${currentYear}`);
  });
});

router.post('/bankholidays/:company_id?/import/', (req, res) => {
  const model = req.app.get('db_model'),
...
router.post('/bankholidays/:company_id?/delete/:bankHolidayId/', function(req, res){
  const model = req.app.get('db_model');
  const currentYear = getCurrentYear({req});
