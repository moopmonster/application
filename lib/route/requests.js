...
router.get('/', function(req, res){
    Promise.join(
        ...
        function(my_leaves_grouped, to_be_approved_leaves, company){

            if(req.originalUrl=="/requests_approve/")
            {
...
function leave_request_action(args) {
    ...
    .then(function(processed_leave){
      ...
      return res.redirect_with_session('../../requests_approve/');
    })
...
router.post('/reqdownload/', function(req, res) {
  ...
  var request_id = validator.trim( req.param('request') );
  Promise.try(function() {return req.user.promise_leaves_to_be_processed();})
  .then(function(leaves) {
