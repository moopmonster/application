...
function leave_request_action(args) {
    ...
    .then(function(processed_leave){
      ...
      if(processed_leave.leave_type.is_adjustment == 0)
      {
...
router.post('/cancel/', function(req, res){
  ...
  .then(function(leave){
    ...
    if(leave.leave_type.is_adjustment == 0)
    {
      /* update google calendar event */
      var gcal = new GcalEvent();      
...
router.post('/upload/', upload.any(), function(req,res) {
  ...
  var model = req.app.get('db_model'),
      ...
      request_id = validator.trim( req.param('request') ),
      userid = validator.trim( req.param('user') );

  let
    fileUpload = _.find(req.files,files => files.fieldname == 'attachment_'+request_id);

  // Promise.try(function() {return req.user.promise_my_active_leaves_ever();}) //Leave not found if upload attachment for someone else
  Promise.try(function() {
    return model.User.findOne({
      where   : {id : userid},
    })
    .then(user => user.promise_my_active_leaves_ever())
  })
  .then(function(leaves) {
  ...
  .then(function(leave_to_upload) {
    var newname = randomid +"-"+ fileUpload.originalname;
    var newpath = "./attachments/" + newname;
    fs.rename(fileUpload.path, newpath, function(err) {if(err) throw err;});
