...
router.post('/bookleave/', upload.single("attachment"), function(req, res){
    ...
    Promise.join (
      ...
      (users, company, valid_attributes) => {
        ...
        var employee = users[valid_attributes.user] || req.user,
          leave_type = company.leave_types[valid_attributes.leave_type];
        ...
        // Beware! Attachment attribute is validated in here rather than in validator/leave_request.js
        if (req.file) {
          var newname = valid_attributes.from_date +"-"+ req.file.originalname
          var newpath = "./attachments/" + newname;
          fs.rename(req.file.path, newpath, function(err) {if(err) throw err;});
          valid_attributes["attachment"] = validator.trim(newname);
        }
