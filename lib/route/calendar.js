...
router.post('/bookleave/', upload.single("attachment"), function(req, res){
    ...
    Promise.join (
      ...
      req.user.get_company_with_all_leave_types(companyID),
      ...
      (users, company, valide_attributes) => {
        ...
        if (req.file) {
          var newname = valide_attributes.from_date +"-"+ req.file.originalname
          var newpath = "./attachments/" + newname;
          fs.rename(req.file.path, newpath, function(err) {if(err) throw err;});
          valide_attributes.attachment = newpath; // adding the attachment here instead of in validator
        }

        //If ticked 'Work From Home', will append 'WFH -' in front of the employee comment
        if (req.param('wfh')){
          valide_attributes.employee_comment = 'WFH - ' + valide_attributes.employee_comment;
        }
